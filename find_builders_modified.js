...
let allFixturesData = [];
let isLoading = false;
let selectedLeagues = new Set(Object.keys(JSON_FILES));
let criteriaCount = 0;
const MAX_CRITERIA = 10;

// DOM elements
const form = document.getElementById('builderFinderForm');
...
function handlePresetClick(e) {
  ...
  if (preset === 'all') {
    ...
    checkboxes.forEach(cb => cb.checked = LEAGUE_GROUPS.top5.includes(cb.value));
  } else if (preset === 'croatian') {
    selectedLeagues = new Set(LEAGUE_GROUPS.croatian);
    checkboxes.forEach(cb => cb.checked = LEAGUE_GROUPS.croatian.includes(cb.value));
  }
}
...
function addCriteriaSection() {
  if (criteriaCount >= MAX_CRITERIA) {
    showError(`Maximum ${MAX_CRITERIA} criteria allowed`);
    return;
  }
  ...
  criteriaDiv.innerHTML = `
    <div class="criteria-header">
      <h4>Criteria ${criteriaCount}</h4>
      ${criteriaCount > 1 ? `<button type="button" class="btn-remove" onclick="removeCriteriaSection('${criteriaId}')">
    ...
    <div class="filter-group">
      ...
      <div class="filter-item">
        ...
        <select class="threshold-select" data-criteria="${criteriaId}" disabled required>
          <option value="">Select threshold</option>
        </select>
      </div>
      <div class="filter-item">
        <label>Over/Under</label>
        <select class="overunder-select" data-criteria="${criteriaId}" required>
  ...
  const categorySelect = criteriaDiv.querySelector('.category-select');
  categorySelect.addEventListener('change', (e) => updateThresholdOptions(e.target));
}

function removeCriteriaSection(criteriaId) {
  const section = document.getElementById(criteriaId);
  if (section) {
    section.remove();
    criteriaCount--;
    // Renumber remaining criteria
    document.querySelectorAll('.criteria-section').forEach((section, index) => {
      section.querySelector('h4').textContent = `Criteria ${index + 1}`;
    });
  }
}

function updateThresholdOptions(categorySelect) {
  const criteriaId = categorySelect.dataset.criteria;
  const thresholdSelect = document.querySelector(`.threshold-select[data-criteria="${criteriaId}"]`);
...
// Get all criteria from form
function getAllCriteria() {
  const criteria = [];
  document.querySelectorAll('.criteria-section').forEach(section => {
    const criteriaId = section.id;
    const category = section.querySelector('.category-select').value;
    const threshold = parseFloat(section.querySelector('.threshold-select').value);
    const overUnder = section.querySelector('.overunder-select').value;

    if (category && !isNaN(threshold) && overUnder) {
      criteria.push({ category, threshold, overUnder });
    }
  });
  return criteria;
}

// Load all data from JSON files (same as find_bets)
...
async function handleFormSubmit(e) {
  ...
  // Validate inputs
  if (!fixtureDate) {
    showError('Please select a fixture date');
    return;
  }

  if (criteria.length === 0) {
    showError('Please add at least one criteria');
    return;
  }
  ...
  try {
    const opportunities = analyzeFixturesForDate(fixtureDate, lastMatches, criteria, minSuccessRate);
    renderResults(opportunities, fixtureDate, lastMatches, criteria, minSuccessRate);
  } catch (error) {
...
function analyzeTeamMatchesMultiCriteria(matches, criteria) {
  ...
  const matchResults = matches.map(match => {
    // Check if ALL criteria pass for this match
    let allCriteriaMet = true;
    ...
    return {
      ...match,
      isSuccess: allCriteriaMet,
      criteriaResults: criteriaResults,
      // For display - show values for all criteria
      displayValues: criteria.map(c => {
        const dataKey = `total${c.category.charAt(0).toUpperCase() + c.category.slice(1)}`;
        return `${c.category}: ${match[dataKey]}`;
      }).join(', ')
    };
  });
...
function createSummaryCard(opportunities, fixtureDate, criteria, minSuccessRate) {
  ...
  const criteriaText = criteria.map((c, i) => 
    `Criteria ${i + 1}: ${c.category} ${c.overUnder} ${c.threshold}`
  ).join('\n');

  if (opportunities.length === 0) {
    header.innerHTML = `
      <i class="fas fa-chart-line"></i>
      <h3>No Builder Opportunities Found</h3>
    `;
  } else {
    header.innerHTML = `
      <i class="fas fa-chart-line"></i>
  ...
  if (opportunities.length === 0) {
    body.innerHTML = `
      <div class="no-opportunities">
        ...
        <p>No fixtures found for ${fixtureDate} that meet all your criteria with ${minSuccessRate}%+ success rate.</p>
    ...
    body.innerHTML = `
      <div class="summary-stats">
        ...
        <div class="summary-stat">
          ...
          <div>
            ...
            <div class="stat-value">${criteria.length} condition${criteria.length === 1 ? '' : 's'}</div>
          </div>
        </div>
        <div class="summary-stat">
      ...
      <div class="builder-criteria">
        <strong>Builder Criteria:</strong>
        <ul>
          ${criteria.map((c, i) => `<li>Criteria ${i + 1}: ${c.category} ${c.overUnder} ${c.threshold}</li>`).join('')}
        </ul>
      </div>
    `;
  }
...
// Create opportunity card exactly like find_bets with individual match indicators
function createOpportunityCard(opportunity, criteria) {
  const card = document.createElement('div');
  card.className = 'card fade-in fixture-card';

  const { analysis, fixture } = opportunity;
  ...
  header.innerHTML = `
    <div class="fixture-header">
      ...
      <div class="success-badge ${successRateClass}">${analysis.combinedSuccessRate}%</div>
    </div>
  `;
  ...
  body.innerHTML = `
    <div class="team-analysis">
      <div class="team-section">
        <h4><i class="fas fa-home"></i> ${analysis.homeTeam} Form</h4>
        ...
        <div class="form-indicators">
          ${analysis.homeAnalysis.matchResults.map(match => 
            `<span class="form-indicator ${match.isSuccess ? 'success' : 'fail'}" 
                   title="${match.homeTeam} vs ${match.awayTeam} (${match.date.toLocaleDateString()})\n${match.displayValues}\n${match.isSuccess ? '✅ All criteria met' : '❌ Not all criteria met'}">
              ${match.isSuccess ? '✓' : '✗'}
            </span>`
          ).join('')}
        </div>
      ...
      <div class="team-section">
        <h4><i class="fas fa-plane"></i> ${analysis.awayTeam} Form</h4>
        <div class="team-stats">
    ...
    <div class="combined-analysis">
      ...
      <div class="combined-stats">
        ...
        <div class="stat-item">
          <span class="stat-label">Builder Criteria</span>
...
function setLoading(loading) {
  isLoading = loading;
  if (loading) {
    findBuildersBtn.disabled = true;
    btnText.classList.add('hidden');
    btnLoading.classList.remove('hidden');
  } else {
    findBuildersBtn.disabled = false;
